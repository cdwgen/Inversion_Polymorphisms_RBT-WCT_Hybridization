...
# Read the VCF file
vcf <- read.vcfR("../00_input_vcfs/Omy13_inversion.recode.vcf")
...
p <- ggplot(pca_df, aes(x = Axis1, y = Axis2, color = population)) +
  ...
  theme_classic() +
  labs(title = "Omy13 Interspecific",
...
write.csv(pca_df, "PCA_Omy13_interspecific_coordinates.csv", row.names = FALSE)
...
# Save 3D plot
saveWidget(p_interactive_3D, file = "PCA_Omy13_interspecific_Interactive_3D.html")