...
# Read the VCF file
vcf <- read.vcfR("../00_input_vcfs/Omy17_inversion.recode.vcf")
...
p <- ggplot(pca_df, aes(x = Axis1, y = Axis2, color = population)) +
  ...
  theme_classic() +
  labs(title = "Omy17 Interspecific",
...
write.csv(pca_df, "PCA_Omy17_interspecific_coordinates.csv", row.names = FALSE)
...
# Save 2D plot
saveWidget(p_interactive_2D, file = "PCA_Omy13_interspecific_Interactive_2D.html")
...
# Save 3D plot
saveWidget(p_interactive_3D, file = "PCA_Omy17_interspecific_Interactive_3D.html")