...
# Read the VCF file
vcf <- read.vcfR("RBT_v_WCT.recode.vcf")
...
popmap <- read.table("popmap_RBT_v_WCT.txt", header = FALSE, stringsAsFactors = FALSE)
...
write.csv(pca_df, "PCA_RBT_coordinates.csv", row.names = FALSE)
...
# Save 2D plot
saveWidget(p_interactive_2D, file = "PCA_RBT_Interactive_2D.html")
...
# Save 3D plot
saveWidget(p_interactive_3D, file = "PCA_RBT_Interactive_3D.html")