...
## Files provided in this directory are examples using Arlee

## First I generated a fasta of all markers for each population-specific map and the global map in excel

## For each map I wanted to plot, I took the complete map info and put it in a text file
## I then ran this script to convert the map information into a TSV: 
...
## Next I aligned markers to the Arlee reference for each population similar to below:

module load minimap2 samtools bedtools

minimap2 -x sr -a -t 8 ~/UMT_Redband/01_data/00_Reference_Genome/norm_IRT_061722.fa Global_all_markers.fasta > Global.sam

samtools view -bS Global.sam | samtools sort -o Global.sorted.bam

samtools index Global.sorted.bam

samtools view -b -q 20 -F 2308 Global.sorted.bam > Global.filtered.bam

bedtools bamtobed -i Global.filtered.bam > Global.bed


## Finally to plot, I ran this script:
